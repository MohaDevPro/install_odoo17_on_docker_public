...
# Create user with superuser, create role, and create DB privileges
# docker exec -i $CONTAINER_NAME psql -U odoo -d postgres -c "CREATE USER $USERNAME WITH CREATEDB CREATEROLE;"
docker exec -i $CONTAINER_NAME psql -U postgres -d postgres -c "CREATE USER $NAME WITH CREATEDB CREATEROLE PASSWORD '$PASSWORD';"
